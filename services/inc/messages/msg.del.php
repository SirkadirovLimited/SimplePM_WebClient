<?php
	...
	if ($db_res->num_rows === 1){
		...
		if ($message["from"] == $_SESSION["uid"]){
			if (!$db->query("UPDATE `spm_messages` SET `delFrom` = '1' WHERE `id`='" . (int)$_GET['del'] . "';"))
		...
		elseif ($message["to"] == $_SESSION["uid"]){
			if (!$db->query("UPDATE `spm_messages` SET `delTo` = '1' WHERE `id`='" . (int)$_GET['del'] . "';"))
				die('<strong>Данное сообщение удалить не возможно!</strong>');
			...
			die('<strong>Хорошая попытка, но глупая! SQL-инъекция заблокирована!</strong>');
		}
